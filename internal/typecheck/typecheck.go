...
func New(schema *syn.Schema, settings Settings) *typechecker {
	return &typechecker{
		...
		info: Info{
			...
			ArgResolutions:      make(map[*syn.Value]*syn.NamePat),
...
type Info struct {
	...
	ArgResolutions  map[*syn.Value]*syn.NamePat
...
func (tcx *typechecker) unusedBindings() map[*syn.NamePat]struct{} {
	...
	// remove bindings that are referenced by name expressions
	for _, binding := range tcx.info.NameResolutions {
		delete(unusedBindings, binding)
	}

	// remove bindings that are referenced by $var patterns
	for _, binding := range tcx.info.VarPatResolutions {
		delete(unusedBindings, binding)
	}

	// remove bindings that are referenced from a graphql query document
	for _, binding := range tcx.info.ArgResolutions {
		delete(unusedBindings, binding)
	}
